...
import { getMovie, getimages, getCredits, getVideos, getSimilarMovies, displayMovies, displayVideos, displayImages, displayPeople } from "./api-functions.js";
...
let {title, vote_average, vote_count, runtime, release_date, genres, overview, original_title
...
if(!overview){
  ...
  overview = (enMovieData.overview)? enMovieData.overview : '영화 정보가 없습니다'
}
...
qySel('.movie-title').innerText = title
...
qySel('.release_date').innerText = release_date
qySel('.genres small').innerText = genreString
qySel('.overview').innerText = overview
qySel('.orginal_title').innerText = original_title
