...
qySel('.search-input').addEventListener('input', async (e) => {
  ...
  timeoutId = setTimeout(async () => {
    controller.abort() /* fetch누적 오류때문에 사용 */
...
const setGenreBtns = () => {
  for (let genreNum in genreList) {
    qySel('.genres-btns').insertAdjacentHTML('beforeend', `
...
const observer = new IntersectionObserver(async ([entry]) => {
  if (entry.intersectionRatio > .1 && isPending === false) { // 세로기준 화면에 10분의 1만큼 안쪽으로 들어왔을 때
    isPending = true
    page++
